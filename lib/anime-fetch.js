...
export async function fetchWithRetry(url, options = {}, retries = 3) {
    for (let attempt = 1; attempt <= retries; attempt++) {
        try {
            logDebug(`request ${attempt}/${retries} -> ${url}`)
            const res = await fetch(url, options)
            if (!res.ok) {
                const body = await res.text()
                logDebug(`response ${res.status} from ${url} (attempt ${attempt}/${retries}) -> ${body}`)
                if (res.status === 502 && attempt < retries) {
                    const delay = 5000 + Math.floor(Math.random() * 10001)
                    console.warn(
                        chalk.yellow('[WARN]') +
                        ` attempt ${attempt} got 502 for ${url}, retrying in ${delay}ms`
                    )
            ...
            logDebug(`success ${res.status} from ${url}`)
            return res
        } catch (err) {
            const detail = err instanceof Error ? err.message : String(err)
            logDebug(`error on attempt ${attempt}/${retries} for ${url}: ${detail}`)
            if (attempt === retries) throw err
            if (detail.startsWith('HTTP 502')) {
                const delay = 5000 + Math.floor(Math.random() * 10001)
                console.warn(
                    chalk.yellow('[WARN]') +
                    ` attempt ${attempt} failed with 502 for ${url}, retrying in ${delay}ms`
                )
                await new Promise(resolve => setTimeout(resolve, delay))
                continue
            }
            const delay = 1000 * Math.pow(2, attempt - 1)
            console.warn(chalk.yellow('[WARN]') + ` attempt ${attempt} failed for ${url}, retrying in ${delay}ms (${detail})`)
            await new Promise(resolve => setTimeout(resolve, delay))
        }
    }
}
